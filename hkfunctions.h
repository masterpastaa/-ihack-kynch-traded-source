...
FVector hkfunctions::W2S(FVector worldloc, FRotator camrot)
{
	...
	FVector vDelta = worldloc - hk_renderbasics::LocalController->PlayerCameraManager->GetCameraLocation();
	FVector vTransformed = FVector(vDelta.Dot(vAxisY), vDelta.Dot(vAxisZ), vDelta.Dot(vAxisX));

	float fov_angle = hk_renderbasics::LocalController->PlayerCameraManager->GetFOVAngle();
...
inline SDK::FVector hkfunctions::GetBoneLocByIdx(PVOID mesh, int id)
{
	...
	fGetBoneMatrix(mesh, hk_renderbasics::myMatrix, id);

	return SDK::FVector(hk_renderbasics::myMatrix->M[3][0], hk_renderbasics::myMatrix->M[3][1], hk_renderbasics::myMatrix->M[3][2]);
}
...
inline void hkfunctions::DrawAimingEnemy(SDK::AFortPlayerPawn* Target)
{
	...
	if (hk_renderbasics::LocalController->ProjectWorldLocationToScreen(target_loc, false, &worldpos)) {
...
FRotator inline LimitRotation(FRotator rStartRotation, FRotator rEndRotation)
{
	...
	rDeltaRotation = hk_renderbasics::MathLib->STATIC_NormalizedDeltaRotator(rEndRotation, rStartRotation);
...
bool hkfunctions::GetBoneScreen(ACharacter* pActor, int nBone, FVector2D* vBonePos)
{
	...
	if (!hk_renderbasics::LocalController->ProjectWorldLocationToScreen(vWorldPos, false, vBonePos))
		return false;
...
inline void hkfunctions::SetAim(SDK::APlayerCameraManager* MyCamera, SDK::AFortPlayerPawn* Target)
{
	...
	fDistance = hk_renderbasics::MathLib->STATIC_VSize(Difference);
	float TimeScale = 0.0193f;
	short Ping = hk_renderbasics::LocalController->Pawn->PlayerState->Ping * 4;
	float ScaleValue = (float)(Ping)+(float)(fDistance * TimeScale);
	if (ScaleValue < 1.0f) ScaleValue = 1.0;
	float fVelocity;
	Velocity = hk_renderbasics::LocalController->GetVelocity();
	fVelocity = hk_renderbasics::MathLib->STATIC_VSize(Velocity);
	if (fVelocity > 90.0f)
	{
	...
	//get aim rotation
	AimRotation = hk_renderbasics::MathLib->STATIC_FindLookAtRotation(CamLocation, AimLocation);
	...
	//set aim
	AimRotation.Pitch = (AimRotation.Pitch - CamRotation.Pitch) / hk_renderbasics::LocalController->InputPitchScale;
	AimRotation.Yaw = (AimRotation.Yaw - CamRotation.Yaw) / hk_renderbasics::LocalController->InputYawScale;

	hk_renderbasics::LocalController->AddPitchInput(AimRotation.Pitch);
	hk_renderbasics::LocalController->AddYawInput(AimRotation.Yaw);
...
{
	FRotator CamRotation = hk_renderbasics::LocalController->PlayerCameraManager->GetCameraRotation();
...
{
	if (!Target || !hk_renderbasics::LocalController->AcknowledgedPawn || !hk_renderbasics::LocalController->AcknowledgedPawn->CurrentWeapon || !hk_renderbasics::LocalController) return false;

	FVector tmp = { 0,0,0 };

	auto fLineOfSight = ((BOOL(__fastcall*)(PBYTE, SDK::AFortPlayerPawn*, FVector*))(fnLineOfSightTo));
	return (fLineOfSight, hk_renderbasics::LocalController, Target, &tmp);
}
