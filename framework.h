...
inline void framework::tabinactive(UCanvas* canvas, int x, int y, int w, int h, const FString string, int& tab, int count, bool show_outline)
{
	...
	if ((cursor.x > x) && (cursor.x < x + w) && (cursor.y > y) && (cursor.y < y + h) && (hk_renderbasics::LocalController->WasInputKeyJustPressed(KeyName::Left) & 1))
		tab = count;
...
inline void framework::tab(UCanvas* canvas, int x, int y, int w, int h, const FString string, int& tab, int count, bool show_outline)
{
	...
	if ((cursor.x + 30 > x) && (cursor.x + 30 < x + w) && (cursor.y > y) && (cursor.y < y + h) && (hk_renderbasics::LocalController->WasInputKeyJustPressed(KeyName::Left) & 1))
		tab = count;
...
inline bool framework::checkbox(UCanvas* canvas, int x, int y, int position, const FString string, bool& value)
{
	GetCursorPos(&cursor);
	int w = 15, h = 15;
	if ((cursor.x > position - 30) && (cursor.x < position - 30 + w) && (cursor.y > y) && (cursor.y < y + h) && (hk_renderbasics::LocalController->WasInputKeyJustPressed(KeyName::Left) & 1))
		value = !value;
	else if ((cursor.x > position - 30) && (cursor.x < position - 30 + w) && (cursor.y > y) && (cursor.y < y + h) && (hk_renderbasics::LocalController->WasInputKeyJustPressed(KeyName::Left) & 1))
		value = !value;
...
inline void framework::slider(UCanvas* canvas, int x, int y, int position, const FString string, float& value, float min_value, float max_value)
{
	...
	if ((cursor.x > ix) && (cursor.x < ix + position) && (cursor.y > yi) && (cursor.y < yi + 6) && (hk_renderbasics::LocalController->IsInputKeyDown(KeyName::Left)))
...
inline void framework::movement(int& x, int& y, int w, int h)
{
	...
	if ((hk_renderbasics::LocalController->WasInputKeyJustPressed(KeyName::Left)) < 0 && (cursor.x > x && cursor.x < x + w && cursor.y > y && cursor.y < y + h))
	{
		should_drag = true;
...
inline void framework::render_menu(UCanvas* canvas)
{
	...
	Text(canvas, 175, 204, 0, FString(E(L"Ihack")), options::Aim::memory);
